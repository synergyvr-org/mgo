...
MGO 4.0 **RC3 hasn't been released yet**. This page previews its setup steps so they're ready when it ships. If you're playing the current release (RC2), follow the regular [Onboarding](/start/onboarding) page instead.
{{< /aside >}}
...
The [**Community Shaders**](/performance/community-shaders/) presets are split by GPU brand: open the folder that matches your video card (NVIDIA or AMD), then select either the _Quality_ or _Performance_ preset—one preset total, not one per folder. The _Quality_ presets have a fairly significant impact on your outdoor frame rate. Whether that's worth it is up to you. (And as the folder name says, you can press <kbd>END</kbd> in game to customize Community Shaders further.)
